...
class Song:
    ...
    def __lt__(self, other):
        return self.key_namn < other.key_namn
...
def binary_search(the_list, key):
   ...
   while low <= high and not found:
      middle = (low + high)//2
      if the_list[middle].key_namn == key:
         found = True
      else:
         if key < the_list[middle].key_namn:
            high = middle - 1
         else:
            low = middle + 1
   return found
...
def main():
    ...
    lista=lista[0:500000]
    n = len(lista)
    print("Antal element =", n)

    sista = lista[n-1]
    testkey = sista.key_namn

    linjtid = timeit.timeit(stmt = lambda: linsok(lista, testkey), number = 100)
